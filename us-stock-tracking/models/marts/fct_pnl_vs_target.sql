...
portfolio_value as (
    select
        ...
        sum(best_price_trade) as total_best_price_trades,
        sum(worst_price_trade) as total_worst_price_trades,
        sum(middle_price_trade) as total_middle_price_trades
    from pnl_by_desk
    group by 1,2,3
),

-- Calculate total portfolio value across all desks and regions for each daily_task_metrics
...
-- Calculate actual allocation and compare with targets
allocation_variance as (
    select
        ...
        pv.portfolio_value_usd/nullif(tp.total_portfolio_value_usd, 0) as actual_location,
