...
-- calculate pnl (profit & loss) for matched_trades
matched_trades_pnl as (
    select
        m.*,
        (m.sell_price - m.buy_price)*m.quantity as pnl,
        (m.sell_price - m.buy_price)*m.quantity as pnl_usd -- same as usd since already in usd
    from matched_trades m
),

-- calculate pnl for individual trades
trades_pnl as (
    select 
        t.trade_id,
        t.trade_date,
        ...
        -- calculate % differences from market price
        case
        ...
        case
            when t.trade_type = 'BUY' then
                case
                    when t.price < s.close_price then 'Better than open'
                    when t.price > s.close_price then 'Worse than open'
                    else 'Equal to open'
                end
            when t.trade_type = 'SELL' then
                case
        ...
        case
            when t.trade_type = 'BUY' then
                case
                    when t.price < s.open_price and t.price < s.close_price then round(((s.close_price - t.price)/s.close_price)*100,2)
                    when t.price > s.open_price and t.price > s.close_price then round(((s.close_price - t.price)/s.close_price)*100,2)
                    else round(((s.close_price - t.price)/s.close_price)*100,2)
                end
            when t.trade_type = 'SELL' then
                 case
                    when t.price > s.open_price and t.price > s.close_price then round(((t.price - s.open_price)/s.open_price)*100,2)
                    when t.price < s.open_price and t.price < s.close_price then round(((t.price - s.open_price)/s.open_price)*100,2)
                    else round(((t.price - s.open_price)/s.open_price)*100,2)
                end
